...
async function confirmAddDeduction() {
  ...
  // Persist edits to code-based deductions immediately, same as new additions do.
  if (
    disbursementId &&
    editingDeductionIndex.value !== -1 &&
    !isManual &&
    deductionForm.value.deduction_code_id
  ) {
    try {
      const token = authStore.admin ? authStore.adminToken : authStore.token
      const res = await api.put(
        `/api/barangay/deductions/${editingDeductionId.value}`,
        {
          deduction_code_id: deductionForm.value.deduction_code_id,
          gross_vat_inc: deductionForm.value.grossVatInc || grossAmount,
          description: deductionForm.value.description || '',
        },
        { headers: { Authorization: `Bearer ${token}`, Accept: 'application/json' } },
      )

      // If the backend recalculates amounts, prefer its response for the local row
      const saved = res.data?.data
      if (saved) {
        const mapped = mapApiDeductionToRow(saved)
        store.forms.expense.deductions = (store.forms.expense.deductions || []).map((d, i) =>
          i === editingDeductionIndex.value ? mapped : d,
        )
      ...
      console.error('Failed to update deduction:', e)
      $q.notify({ type: 'negative', message: 'Failed to update deduction.', position: 'top' })
      return
    }
  }
