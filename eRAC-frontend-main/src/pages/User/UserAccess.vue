<template>
  <q-page class="q-pa-md user-access-page">
      ...
      <div class="row items-center justify-between">
        <div class="text-h6 text-weight-medium">
          ...
          <span class="text-caption q-ml-sm"
            >({{ filteredUsers.length }} users from {{ currentUserBarangay }})</span
          >
        </div>
        <q-btn
          icon="refresh"
    ...
    <q-banner v-if="!hasValidToken" class="bg-orange text-white q-mb-md" icon="warning">
    ...
    <q-banner v-if="false" class="bg-grey-3 text-dark q-mb-md" icon="info">
      ...
      <div>
        Users count: {{ users.length }}, Type: {{ typeof users }}, Is Array:
        {{ Array.isArray(users) }}
      </div>
      <div>
        Filtered count: {{ filteredUsers.length }}, Current barangay: {{ currentUserBarangay }}
      </div>
    </q-banner>
    ...
    <q-card flat bordered>
      ...
      <div v-if="!loading && filteredUsers.length === 0" class="q-pa-lg text-center">
        ...
        <div class="text-body2 text-grey-6">
          {{
            users.length === 0
              ? 'No users available in your barangay.'
      ...
      <q-card style="min-width: 400px">
        <q-card-section class="q-pb-none">
          <div class="row items-center justify-between">
            ...
            <q-btn flat dense round icon="close" @click="closeAccessModal" />
          </div>
...
export default {
  ...
  computed: {
    ...
    filteredUsers() {
      ...
      let filtered = this.users.filter((user) => user.barangay_name === this.currentUserBarangay)
  ...
  methods: {
    ...
    validateAccess() {
      const hasAnyPermission = Object.values(this.accessModal.permissions).some(
    ...
    async saveAccess() {
      ...
      try {
        ...
        const response = await api.post(
          `/api/barangay/users/${this.accessModal.selectedUser.id}/permissions`,
          {
            permissions,
          },
          config,
        ...
        if (
          ...
          throw new Error(
            `Unexpected response format. Expected success: true, got: ${response.data.success}`,
          )
        }
      } catch (error) {
