      ...
      <div class="login-container q-pt-md">
        <div class="header-section">
          <div class="office-info">
            <q-img src="src/assets/tagumlogo.png" class="logo-image" contain
          ...
          <q-card class="signup-card">
            ...
            <q-stepper v-model="step" color="green" keep-alive>
              ...
              <q-step :name="1" title="Personal Info" icon="person" :done="step > 1">
                <div class="row q-col-gutter-md">
                  ...
                  <div class="col-6">
                    ...
                    <q-input outlined dense bg-color="white" v-model="firstName" label="Firstname" color="green"
                      class="q-mb-sm" :error="showStep1Validation && !firstName"
                      :error-message="showStep1Validation && !firstName ? 'First name is required' : ''"
                      @keyup.enter="validateStep1" />
                    <q-input outlined dense bg-color="white" v-model="middleName" label="Middlename" color="green"
                      class="q-mb-sm" :error="showStep1Validation && !middleName"
                      :error-message="showStep1Validation && !middleName ? 'Middle name is required' : ''"
                      @keyup.enter="validateStep1" />
                    <q-input outlined dense bg-color="white" v-model="lastName" label="Lastname" color="green"
                      class="q-mb-md" :error="showStep1Validation && !lastName"
                  ...
                  <div class="col-6">
                    ...
                    <q-select outlined dense bg-color="white" v-model="barangay" :options="barangayOptions"
                      label="Select Barangay" color="green" class="q-mb-sm" emit-value map-options option-label="name"
                      option-value="value" :error="showStep1Validation && !barangay"
                    ...
                    <q-select outlined dense bg-color="white" v-model="position" :options="positionOptions"
                      label="Select Position" color="green" class="q-mb-md" emit-value map-options option-label="name"
                      option-value="value" :error="showStep1Validation && !position"
                      :error-message="showStep1Validation && !position ? 'Position is required' : ''"
                      @keyup.enter="validateStep1" />
                    <q-uploader v-model="uploadedFiles" label="Drag and drop photo or click to browse"
                      accept=".jpg,.jpeg,.png,.gif" max-file-size="5242880" max-files="1" style="width: 100%"
                      color="green" flat bordered bg-color="white" @added="onFileAdded" @rejected="onFileRejected"
                      ...
                      <template v-slot:header="scope">
                        <div class="row no-wrap items-center q-pa-sm">
                          ...
                          <div class="row items-center q-gutter-sm q-ml-md">
                            <!-- Add Button -->
                            <q-btn icon="add" round dense flat color="white" v-if="scope.canAddFiles"
                              :disable="uploadedFiles.length > 0" @click="scope.addFiles()">
                              <q-uploader-add-trigger />
                            </q-btn>
                            <!-- Delete Button -->
                            <q-btn icon="delete" round dense flat color="red" :disable="uploadedFiles.length === 0"
                      ...
                      <template v-slot:list="scope">
                        <q-list separator>
                          <q-item v-for="file in scope.files" :key="file.name">
                            <q-img v-if="photoUrl" :src="photoUrl" style="max-width: 200px; margin-top: 10px"
              ...
              <q-step :name="2" title="Account Info" icon="lock" :done="step > 2">
                <div class="row q-col-gutter-md">
                  <div class="col-6">
                    <q-input outlined dense bg-color="white" v-model="email" label="Email" color="green"
                ...
                <div class="row q-col-gutter-md q-mt-sm">
                  <div class="col-6">
                    <q-input outlined dense bg-color="white" v-model="password" label="Password"
                      :type="isPasswordVisible ? 'text' : 'password'" color="green"
                  ...
                  <!-- Confirm Password-->
                  <div class="col-6">
                    <q-input outlined dense bg-color="white" v-model="confirmPassword" label="Confirm Password"
                      :type="isPasswordVisible2 ? 'text' : 'password'" color="green"
                      :error="showStep2Validation && (!confirmPassword || password !== confirmPassword)"
                      :error-message="getConfirmPasswordErrorMessage()">
                      <template #append>
                        <q-icon :name="isPasswordVisible2 ? 'visibility_off' : 'visibility'" class="cursor-pointer"
                ...
                <q-stepper-navigation class="row justify-between q-mt-md">
                  ...
                  <q-btn @click="goToOtp" color="green" label="Verify Email" :loading="isLoading"
                    :disable="isLoading" />
              ...
              <q-step :name="3" title="OTP" icon="verified_user" :done="step > 3">
                <div class="otp-verification-section">
                  ...
                  <div v-if="isSending" class="text-center q-pa-lg">
                  ...
                  <!-- Show OTP form when not loading -->
                  <div v-else>
                    ...
                    <div class="otp-input-container">
                      ...
                      <div class="row justify-center q-col-gutter-sm">
                        <div class="col-auto">
                          <q-input outlined dense bg-color="white" v-model="otpCode" label="OTP Code" color="green"
                            class="otp-input" maxlength="6" @keyup.enter="handleSubmit">
                            <template v-slot:prepend>
                              <q-icon name="lock" />
                            </template>
                          </q-input>
                    ...
                    <div class="text-center q-mt-md">
                      ...
                      <q-btn flat color="green" label="Resend Code" @click="resendOtp" class="text-caption" />
                    </div>
                  </div>
                </div>
...
@media (max-width: 1023px) {
  ...
  .photo-section {
    display: none;
    /* Hide photo section on mobile/tablet */
  }

  .login-section {
    flex: 1;
    width: 100%;
    /* Login section takes full width */
    padding: 1rem;
  }
